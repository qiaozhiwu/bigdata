...
from client.dp_mechanism import cal_client_sensitivity, simple_composition, laplace_mechanism,gaussian_mechanism
from torch.utils.data import DataLoader


def grnn_lenet_attack(command_str):
    # 使用GRNN攻击LeNet，命令行的格式同Fed
    # 将batch_size为1，不进行梯度裁剪的LeNet第一轮训练梯度进行差分隐私后输入GRNN以模拟差分隐私面对的最坏情况
    ...
    args = args_parser(command_str.split())
    k = args.epochs
    lr = args.lr
    ...
    sensitivity = cal_client_sensitivity(lr, dp_clip, batchsize)
    epsilon, delta = simple_composition(k, total_epsilon,total_delta)
    # if dataset == 'lfw':
    #     shape_img = (32, 32)
    # elif dataset == 'mnist':
    #     shape_img = (28, 28)
    ...
    save_path = os.path.join(root_path,
                             f"Results\\GRNN\\GRNN-{net_name}-{dataset}-S{shape_img[0]}-B{str(batchsize).zfill(3)}-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}")  # path to saving results
    print('>' * 10, save_path)
    ...
    train_loader = iter(
        torch.utils.data.DataLoader(dst, batch_size=batchsize, shuffle=True, generator=torch.Generator(device='cuda')))
    ...
    for idx_net in range(num_exp):
        ...
        pred = net(gt_data)
        y = criterion(pred, gt_label.long())
        ...
        if dp_mechanism == "Laplace":
            noise = laplace_mechanism(epsilon, sensitivity, flatten_true_g.shape)
            noise = torch.from_numpy(noise).to(device1)
            flatten_true_g += noise
        elif dp_mechanism == "Gaussian":
            noise = gaussian_mechanism(epsilon, delta, sensitivity, flatten_true_g.shape)
            noise = torch.from_numpy(noise).to(device1)
            flatten_true_g += noise
        ...
        for iters in iter_bar:  # start  optimizing GRNN
            ...
            Gout, Glabel = Gout.cuda(device1), Glabel.cuda(
                device1)  # assign to device1 as global model's input to generate fake gradient
            Gpred = net(Gout)
        ...
        for imidx in range(batchsize):
            ...
            for i in range(min(len(history), plot_num - 1)):
                ...
                plt.title('label=%d' % (history_l[i][imidx]))
            ...
            result_path = save_path + '\\exp-%03d-imidx-%02d-tlabel-%d-Glabel-%d.png' % (
...
if __name__ == '__main__':
    command_str = "--dataset mnist --model LeNet --dp_mechanism no_dp --dp_epsilon 5 --dp_delta 1e-5 --dp_clip 10 --lr_decay 1 --dp_composition Simple"
